...
function doCheckout() {
  ...
  // Save order details to sessionStorage for thank-you page
  const cart = getCart();
  const subtotal = cart.reduce((s, c) => s + (c.price * c.qty), 0);
  const tax = subtotal * 0.08;
  const total = subtotal + tax;
  sessionStorage.setItem('ov_last_order', JSON.stringify({
    items: cart,
...
function initThankYou() {
  const wrap = document.getElementById('orderDetails');
  if (!wrap) return;

  try {
    const order = JSON.parse(sessionStorage.getItem('ov_last_order'));
    if (!order) {
      wrap.innerHTML = '<p style="color:var(--text-dim);text-align:center;">No recent order found.</p>';
      return;
    }
    ...
    wrap.innerHTML = `
      <div style="background:var(--surface);border-radius:var(--radius);border:1px solid var(--border);padding:1.5rem;margin-bottom:1.2rem;">
        <h3 style="font-family:'Bangers',cursive;font-size:1.2rem;color:var(--accent);letter-spacing:2px;margin-bottom:0.6rem;">Order Details</h3>
        <p style="color:var(--text-dim);font-size:0.82rem;margin-bottom:1rem;">Placed on ${orderDate}</p>
        ${order.items.map(item => `
          <div style="display:flex;gap:0.8rem;align-items:center;padding:0.7rem 0;border-bottom:1px solid var(--border);">
            <span style="font-size:1.8rem;">${item.emoji}</span>
            <div style="flex:1;">
              <h4 style="font-size:0.85rem;font-weight:800;">${item.name}</h4>
              <p style="font-size:0.75rem;color:var(--text-dim);">Qty: ${item.qty} × $${item.price.toFixed(2)}</p>
            </div>
            <strong style="color:var(--accent2);font-size:0.95rem;">$${(item.price * item.qty).toFixed(2)}</strong>
          </div>
        `).join('')}
      </div>

      <div style="background:var(--surface);border-radius:var(--radius);border:1px solid var(--accent);padding:1.3rem;">
        <div style="display:flex;justify-content:space-between;padding:0.4rem 0;border-bottom:1px solid var(--border);">
          <span style="color:var(--text-dim);font-size:0.84rem;">Subtotal</span>
          <strong>$${order.subtotal}</strong>
        </div>
        <div style="display:flex;justify-content:space-between;padding:0.4rem 0;border-bottom:1px solid var(--border);">
          <span style="color:var(--text-dim);font-size:0.84rem;">Tax</span>
          <strong>$${order.tax}</strong>
        </div>
        <div style="display:flex;justify-content:space-between;padding:0.4rem 0;border-bottom:1px solid var(--border);">
          <span style="color:var(--text-dim);font-size:0.84rem;">Shipping</span>
          <strong style="color:var(--accent);font-size:0.8rem;">FREE</strong>
        </div>
        <div style="display:flex;justify-content:space-between;padding-top:0.6rem;margin-top:0.3rem;">
          <span style="font-weight:800;font-size:1rem;">Total</span>
          <strong style="font-size:1.25rem;color:var(--accent2);">$${order.total}</strong>
        </div>
      </div>
    `;
  } catch (e) {
    wrap.innerHTML = '<p style="color:var(--text-dim);text-align:center;">Error loading order details.</p>';
  }
}
